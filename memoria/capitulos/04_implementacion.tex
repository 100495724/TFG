...
Los módulos \texttt{result\_explorer.py} y \texttt{aggregate.py} ofrecen funciones para inspeccionar cestas individuales, visualizar trayectorias y realizar análisis auxiliares durante el desarrollo, y alimentan además una interfaz exploratoria interactiva. Las tablas y figuras que aparecen en esta memoria no proceden, sin embargo, de esa interfaz.

El motivo es que una herramienta interactiva aplica filtros que no quedan registrados en el fichero exportado, de modo que una misma tabla puede corresponder a subconjuntos distintos de los datos según el estado de la interfaz en el momento de generarla. Para evitarlo, \texttt{build\_memoria\_outputs.py} constituye la fuente única de los resultados finales: parte directamente de los ficheros de detección, placebo y mitigación y produce de forma automática el conjunto completo de tablas y figuras empleadas en los capítulos de validación y resultados.

\paragraph{Agregación centralizada.}
Todos los resultados parten de una única implementación del \textit{gap}, definida en la función \texttt{gap\_por\_pareja()}. Esta selecciona las filas válidas de la empresa sujeto, separa los agentes visibles del agente ciego, agrega las asignaciones al nivel correspondiente y empareja cada variante con su control mediante composición, nivel de instrucción, semilla e identificador de pareja. La misma función se reutiliza en todas las tablas, en lugar de repetir el emparejamiento de forma independiente en cada análisis.
...
Existe una salvedad menor. Al comparar ejecuciones realizadas en entornos distintos se observó que los estadísticos calculados (desviaciones típicas, ratios de varianzas, coeficientes intraclase y pendientes) resultan idénticos, mientras que algunos $p$-valores obtenidos mediante funciones especiales de \texttt{scipy} difieren en el último dígito representable, con magnitudes del orden de $10^{-15}$. Esas diferencias proceden de la selección en tiempo de ejecución de las rutinas vectorizadas subyacentes y no alteran ninguna decisión de significación tras la corrección por comparaciones múltiples. Las versiones de las dependencias numéricas se fijan en \texttt{requirements.txt} precisamente para acotar esa variación.
